"""create tessot master data tables

Revision ID: b4c5d6e7f8a9
Revises: a3b4c5d6e7f8
Create Date: 2026-06-22 16:10:00.000000

Four reference tables loaded from the Tessot Semiconductor master-data
workbook (one table per sheet), each scoped to a tenant via tenant_id.
Every business column is NOT NULL with a type-based server_default so
seeded rows never carry a NULL. Generated from scripts/_tessot_spec.py.
"""
...
def downgrade() -> None:
    op.drop_index(op.f("ix_tessot_job_posting_data_tenant_id"), table_name="tessot_job_posting_data")
    op.drop_table("tessot_job_posting_data")
    op.drop_index(op.f("ix_tessot_policy_tenant_id"), table_name="tessot_policy")
    op.drop_table("tessot_policy")
    op.drop_index(op.f("ix_tessot_financial_data_tenant_id"), table_name="tessot_financial_data")
    op.drop_table("tessot_financial_data")
    op.drop_index(op.f("ix_tessot_base_data_tenant_id"), table_name="tessot_base_data")
